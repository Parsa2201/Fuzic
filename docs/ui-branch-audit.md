...
- **Playlists and playlist detail:** grid/create/detail/play-all UI exists. Use Paging 3 for long section/list data, replace the fixed two-column grid with `GridCells.Adaptive` after agreeing minimum card width, and wire optional cover selection to an Android photo picker.
