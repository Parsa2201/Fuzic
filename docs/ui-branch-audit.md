...
| Done | P0 | Paging collection | Chat, Search, and Notifications recreated a `Flow<PagingData<…>>` during each recomposition, which could repeatedly reset their `LazyPagingItems` and render an empty state. | Each screen now remembers the Paging flow for the current `PagingData` instance. |
...
| Done (`ui`) | P1 | Logout entry | Choosing `ProfileEntry.Logout` merely opens Settings instead of requesting logout or opening the confirmation dialog. This is misleading and adds an unexpected extra step. | Profile opens Settings with a typed confirmation argument; after successful logout the root navigation graph is cleared and Welcome becomes the only destination. |
| Done | P1 | Music navigation | Album cards previously had a tap affordance despite having no supported destination. | Routing is type-safe and album cards remain visible as required but are not interactive until an album-detail feature is explicitly added. |
...
- **Playlists and playlist detail:** grid/create/detail/play-all UI exists. **Done (`ui`):** adaptive grid and optional Android image cover picker. Pending: Paging 3 once its repository is implemented.
...
- **Player:** **Done (`ui`):** player likes persist through `InteractionRepository` with rollback on failure. Pending backend/player-layer work: continuous progress/visualizer frames, MediaSession, notification, and lock-screen controls.
- **Chat:** **Done (`ui`):** list/detail/share, paging history, read receipts, reverse layout/newest-at-bottom, and scroll-to-new-message behavior are implemented. Realtime/cache behavior remains repository-owned.
